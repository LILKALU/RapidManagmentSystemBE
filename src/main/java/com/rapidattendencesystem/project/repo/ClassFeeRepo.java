...
public interface ClassFeeRepo extends JpaRepository<ClassFee, Integer> {
    ...
    @Query("SELECT cfc.month FROM ClassFee cf JOIN cf.classFeeCourse cfc WHERE cf.student = :student AND cfc.course = :course")
    List<Month> findPayedMonthByStudentAndCourse(@Param("student") Student student, @Param("course") Course course);

    @Query("SELECT cfc.month FROM ClassFee cf JOIN cf.classFeeCourse cfc WHERE cf.student = :student AND cfc.course = :course ORDER BY cfc.month ASC")
    List<Month> findTop1MonthByStudentAndCourseOrderByMonthAsc(@Param("student") Student student, @Param("course") Course course);
